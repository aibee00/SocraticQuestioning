...
from llava.mm_utils import tokenizer_image_token, get_model_name_from_path, get_answer_sqa, get_rationale_sqa, get_smg_prompt, get_input_ids_from_query_sqa
...
def eval_model(args):
    ...
    for i, (prompt_smg, line) in enumerate(tqdm(zip(prompts_smg, questions), total=len(questions))):
        idx = line["id"]
        ...
        if 'image' in line:
            ...
            rationale = get_rationale_sqa(args, model, tokenizer, prompt_smg, images, debug=args.debug)
        else:
            images = None
            rationale = None
        ...
        input_ids = get_input_ids_from_query_sqa(args, line, cur_prompt, tokenizer, model.config)
        input_ids = input_ids.to(device='cuda', non_blocking=True)
        outputs = get_answer_sqa(args, model, tokenizer, input_ids, images)
        print(f"Current question: {cur_prompt}, \n\nOutput: {outputs}\n\n") if args.debug else None

        # prompt for answer
        if False and args.answer_prompter:
